...
public class TitleItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public TitleItemDecoration(Context context, WordAdapter adapter) {
        super();
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (position > -1) {
                if (position == 0) {//等于0的时候绘制title
                    ...
                    if (null != adapter.getItemData(position).firstLetter && !adapter.getItemData(position)
                            .firstLetter.equals(adapter.getItemData(position - 1).firstLetter)) {
    ...
    private void drawTitle(Canvas c, int left, int right, View child, RecyclerView.LayoutParams params, int position) {
        ...
        mPaint.getTextBounds(adapter.getItemData(position).firstLetter, 0, adapter.getItemData(position).firstLetter.length(), mBounds);
        c.drawText(adapter.getItemData(position).firstLetter,
    ...
    public void onDrawOver(Canvas c, final RecyclerView parent, RecyclerView.State state) {
        ...
        String tag = adapter.getItemData(position).firstLetter;
        ...
        if ((position + 1) < adapter.getItemCount()) {
            ...
            if (null != tag && !tag.equals(adapter.getItemData(position + 1).firstLetter)) {
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        if (position > -1) {
            ...
            if (position == 0) {
                ...
                if (null !=adapter.getItemData(position).firstLetter &&
                        !adapter.getItemData(position).firstLetter.equals(adapter.getItemData(position - 1).firstLetter)) {
